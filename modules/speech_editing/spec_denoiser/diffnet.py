...
class ResidualBlock_wo_diff(nn.Module):
    def __init__(self, encoder_hidden, residual_channels, dilation):
        super().__init__()
        self.dilated_conv = Conv1d(residual_channels, 2 * residual_channels, 3, padding=dilation, dilation=dilation)
        self.diffusion_projection = Linear(residual_channels, residual_channels)
...
class WaveNet(nn.Module):
    def __init__(self, in_dims=80):
        ...
        self.residual_layers = nn.ModuleList([
            ResidualBlock_wo_diff(params.encoder_hidden, params.residual_channels, 2 ** (i % params.dilation_cycle_length))
            for i in range(params.residual_layers)
        ])
        self.skip_projection = Conv1d(params.residual_channels, params.residual_channels, 1)
        self.output_projection = Conv1d(params.residual_channels, in_dims, 1)
    ...
    def forward(self, spec, diffusion_step, cond):
        ...
        #zero spec means cut spec input
        x = torch.zeros_like(x, device=x.device)

        #time pre net: abandon
        #diffusion_step = self.diffusion_embedding(diffusion_step)
        #diffusion_step = self.mlp(diffusion_step)

        #residual_layers: kernel module
        skip = []
        for layer_id, layer in enumerate(self.residual_layers):
            x, skip_connection = layer(x, cond)
            skip.append(skip_connection)
